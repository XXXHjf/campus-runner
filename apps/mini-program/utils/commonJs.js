 ...
 * @param {Number} maxSize - 最大文件大小（字节），默认 2MB
 * @returns {Promise<String>} - 压缩后的文件路径
 */
function compressImageSmart(filePath, maxSize = 2 * 1024 * 1024) {
  return new Promise(async (resolve, reject) => {
    try {
      // 获取原始文件信息
      const originalFileInfo = await new Promise((res, rej) => {
        wx.getFileInfo({
          filePath: filePath,
          success: res,
          fail: rej
      ...
      let currentSize = originalFileInfo.size;
      
      console.log(`[图片压缩] 原始大小: ${(currentSize / 1024 / 1024).toFixed(2)}MB`);
      
      // 如果文件小于等于最大限制，直接返回
      if (currentSize <= maxSize) {
        console.log(`[图片压缩] 文件大小符合要求，无需压缩`);
        resolve(currentPath);
        return;
      }
      ...
      // 第一次压缩 - 使用较高质量 (60)
      console.log(`[图片压缩] 开始第一次压缩 (质量: 60)`);
      const firstCompress = await new Promise((res, rej) => {
        wx.compressImage({
          src: currentPath,
          quality: 60,
          success: res,
      ...
      currentPath = firstCompress.tempFilePath;
      
      // 检查第一次压缩后的大小
      const firstFileInfo = await new Promise((res, rej) => {
        wx.getFileInfo({
      ...
      currentSize = firstFileInfo.size;
      console.log(`[图片压缩] 第一次压缩后: ${(currentSize / 1024 / 1024).toFixed(2)}MB`);
      
      // 如果仍然超过限制，进行第二次压缩 (40)
      if (currentSize > maxSize) {
        console.log(`[图片压缩] 开始第二次压缩 (质量: 40)`);
        const secondCompress = await new Promise((res, rej) => {
          wx.compressImage({
            src: currentPath,
            quality: 40,
            success: res,
        ...
        currentPath = secondCompress.tempFilePath;
        
        const secondFileInfo = await new Promise((res, rej) => {
          wx.getFileInfo({
            filePath: currentPath,
            success: res,
            fail: rej
        ...
        currentSize = secondFileInfo.size;
        console.log(`[图片压缩] 第二次压缩后: ${(currentSize / 1024 / 1024).toFixed(2)}MB`);
        
        // 如果还是超过限制，进行第三次压缩 (20)
        if (currentSize > maxSize) {
          console.log(`[图片压缩] 开始第三次压缩 (质量: 20)`);
          const thirdCompress = await new Promise((res, rej) => {
            wx.compressImage({
              src: currentPath,
              quality: 20,
              success: res,
          ...
          currentPath = thirdCompress.tempFilePath;
          
          const thirdFileInfo = await new Promise((res, rej) => {
            wx.getFileInfo({
              filePath: currentPath,
              success: res,
              fail: rej
          ...
          currentSize = thirdFileInfo.size;
          console.log(`[图片压缩] 第三次压缩后: ${(currentSize / 1024 / 1024).toFixed(2)}MB`);
        }
      }
      ...
      // 最终检查
      if (currentSize > maxSize) {
        console.warn(`[图片压缩] 警告: 压缩后仍超过限制 ${(currentSize / 1024 / 1024).toFixed(2)}MB`);
      } else {
        console.log(`[图片压缩] 压缩完成，最终大小: ${(currentSize / 1024 / 1024).toFixed(2)}MB`);
      }
...
module.exports = {
  ...
  compressImageSmart: compressImageSmart
}
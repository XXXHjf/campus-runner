...
管理端路径和页面映射见 [管理端接口](admin-management.md)，跑腿支付与退款时序见
[跑腿订单与资金流程](../mini-program/order-flow.md)。
...
不使用二手模拟支付或模拟收款。生产默认关闭模拟支付；仅在用户明确要求线上开发测试时
由服务器环境变量启用，并保持到用户明确要求关闭。模拟结果不是实际资金到账。

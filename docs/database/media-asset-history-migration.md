...
> 归档状态：生产迁移观察期结束后，运行时迁移器、巡检脚本和旧字段兜底已从当前代码删除。
> 本文与配套 SQL 只保留作审计记录，不得在当前环境重新部署。当前清理发布见
> [旧兼容链路清理发布说明](legacy-compatibility-cleanup.md)。
...
- `DISCARDED`：经业务确认不再需要的无效或测试图片；旧值已清空且不创建资源。
